...
func (m *Model) SetContentLines(lines []string) {
	m.setContentLines(lines)
	m.longestLineWidth = maxLineWidth(m.lines)
	m.afterSetContent()
}

// SetContentLinesWidth is [Model.SetContentLines], but takes the longest
// line's already-known display width instead of computing it by ansi-width
// scanning every line - the dominant cost of SetContentLines on a large
// buffer (see kage's ui/bench_scroll_test.go), and pure waste for a caller
// that already knows its content's width because it built every line by
// padding/wrapping to a fixed target width itself. longestLineWidth only
// feeds horizontal-scroll math (maxXOffset/HorizontalScrollPercent) and the
// SoftWrap/cut fast-path check in visibleLines, so passing a value that's
// too small can under-clip a wide line - pass a safe upper bound if the
// exact width isn't tracked precisely, or call SetContentLines instead.
func (m *Model) SetContentLinesWidth(lines []string, longestLineWidth int) {
	m.setContentLines(lines)
	m.longestLineWidth = longestLineWidth
	m.afterSetContent()
}

// setContentLines does SetContentLines' line-splitting normalization, shared
// by SetContentLines and SetContentLinesWidth.
func (m *Model) setContentLines(lines []string) {
...
// afterSetContent is the bookkeeping shared by SetContentLines and
// SetContentLinesWidth once m.lines/m.longestLineWidth are settled.
func (m *Model) afterSetContent() {
	m.ClearHighlights()
