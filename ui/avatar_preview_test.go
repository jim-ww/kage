...
// writeTestPNG writes a solid-color square and returns its path.
...
func TestAvatarPreviewSizeGivesUpWhenTiny(t *testing.T) {
	...
	m.updateSizes()
	if cols, rows := m.avatarPreviewSize(); cols != 0 || rows != 0 {
		t.Errorf("avatarPreviewSize() = (%d, %d) in a tiny terminal, want (0, 0)", cols, rows)
	}
	cols, _ := avatarPreviewModel(t, &fakeAvatarPublisher{}).avatarPreviewSize()
	if cols%2 != 0 {
		t.Errorf("avatarPreviewSize() cols = %d, want an even count of cells", cols)
	}
}
